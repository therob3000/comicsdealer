    ...
    for ($j = 0; $j < count($arrayComics); $j++) {
        ...
        if ($layout == 0) {
            ...
            if (is_null($inventario_paquete)) {
                ...
                $codigohtml = $codigohtml . "<h5 class='paquete col-xs-12'>PAQUETE<br><titulo>$nombrePaquete</titulo><br><idioma>$comic_idioma</idioma><br><precio>$$precio_paquete<small> MXN</small></precio></h5></div></a>";
            }
            ...
            $codigohtml = "<div align='center' class='cuadro col-xs-12 col-sm-6 col-md-3 col-lg-3' id='$inventario_id'>
                           <a
                           href='$hrefDetalle' id='cat_detalle'>"
                    . "<div class='image'>"
                    . "<img id='cat_imagen' src=$cat_comic_imagen_mini style='max-width: 100%;max-height: 100%' class='img-rounded img-responsive'>";
            if (is_null($inventario_paquete)) {
...
//$rango = Numero de resultados regresados
    ...
    switch ($orden) {
        ...
        case 1:
            ...
            break;
        case 2:
            $cadena_orden = " ORDER BY cat_comic_numero_compras DESC";
            break;
    }
...
//    $queryCatalogoComics = "
//        SELECT INV.inventario_cat_comic_unique_id FROM inventario AS INV
...
//            
    $queryCatalogoComics = generaQueryGeneral();
    //Si no se ha pasado ningun parametro, por lo general se cargo todo el Catalogo
    if ($idioma == 0 && $compania_id == 0 && $personaje_id == 0) {
        $queryCatalogoComicsCondicion = "
            WHERE
                cat_comic_activo = 1 
    ...
    //Si se ha pasado algun parametro, por lo general se hizo una busqueda especifica
    else {
        switch ($idioma) {
            //1 PARA INGLES
            case 1:
                ...
                if ($compania_id == 0) {
                    if ($personaje_id == 0) {
                        $queryCatalogoComicsCondicion = $query . " GROUP BY inventario_cat_comic_unique_id";
                    } else {
                        $queryCatalogoComicsCondicion = $query . " AND personaje_id = $personaje_id GROUP BY inventario_cat_comic_unique_id";
                    }
                } else {
                    $queryCatalogoComicsCondicion = $query . " AND personaje_compania_id = $compania_id
                    GROUP BY inventario_cat_comic_unique_id";
                }
                break;
            case 2:
                ...
                if ($compania_id == 0) {
                    ...
                    $queryCatalogoComicsCondicion = $query . " AND personaje_compania_id = $compania_id GROUP BY inventario_cat_comic_unique_id";
                }
                break;
            default:
                $query = "
                ...
                if ($compania_id == 0) {
                    ...
                    if ($personaje_id == 0) {
                        ...
                        $queryCatalogoComicsCondicion = $query . " AND personaje_id = $personaje_id AND PERS.personaje_compania_id = $compania_id  GROUP BY inventario_cat_comic_unique_id";
                    }
                }
                break;
        }
...
function obtenerPrecioPaquete($paquete_id){
    $query = "SELECT SUM(inventario_precio_salida) as precio_paquete FROM inventario WHERE inventario_paquete = $paquete_id";
    $queryResultado = mysql_query($query);
    
    return mysql_result($queryResultado, 0, "precio_paquete");
}
