...
$queryPaquetes = "select inventario_cat_comic_unique_id from inventario 
...
if ($num >= 0) {
    for ($i = 0; $i < $num; $i++) {
        $inventario_id[] = mysql_result($queryResultadoIds, $i, "inventario_cat_comic_unique_id");
    }
}

for ($i = 0; $i < count($inventario_id); $i++) {
    //Actualizamos el inventario del comic poniendo existente en 0 
    $queryActInventario = "UPDATE inventario SET inventario_existente = 0 WHERE inventario_existente = 1 AND inventario_cat_comic_unique_id = $inventario_id[$i]";
...
if ($exito) {
    for ($i = 0; $i < count($inventario_id); $i++) {
        ...
        //Actualizamos el catalogo de comics agregando una compra a la columna cat_comic_numero_compras
        $queryComics = "UPDATE cat_comics SET cat_comic_copias = cat_comic_copias - 1, cat_comic_numero_compras = cat_comic_numero_compras + 1 WHERE cat_comic_copias NOT IN (0) AND cat_comic_unique_id = $inventario_id[$i]";
